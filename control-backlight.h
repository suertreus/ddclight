...
class BacklightControl : public Control {
 public:
  static absl::StatusOr<std::optional<BacklightControl>> Probe(absl::string_view output, absl::string_view output_dir);
 ...
 private:
  BacklightControl(std::string name, FDHolder brightness_fd, FDHolder actual_brightness_fd, int max_brightness)
      : Control(std::move(name)), brightness_fd_(std::move(brightness_fd)), actual_brightness_fd_(std::move(actual_brightness_fd)), max_brightness_(max_brightness) {}
