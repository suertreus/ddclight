...
namespace jjaro {
absl::StatusOr<std::unique_ptr<Control>> Control::Probe(const absl::string_view output) {
  ...
  while (true) {
    ...
    if (!ent)
      return absl::NotFoundError(absl::StrCat("no drm output directory found for ", output));
    ...
    if (!bl.ok()) return absl::Status(bl.status().code(), absl::StrCat("failed to probe backlight control for ", output, ": ", bl.status().message()));
    if (*bl) return std::make_unique<BacklightControl>(std::move(**bl));
    auto ddc = I2CDDCControl::Probe(output, output_dir);
    if (!ddc.ok()) return absl::Status(ddc.status().code(), absl::StrCat("failed to probe DDC I2C control for ", output, ": ", ddc.status().message()));
